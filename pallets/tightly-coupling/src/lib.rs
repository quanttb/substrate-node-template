...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn access_storage_from_template(origin: OriginFor<T>) -> DispatchResult {
			...
			// Emit an event.
			Self::deposit_event(Event::StoreAccessed(data));
			// Return a successful DispatchResultWithPostInfo
			Ok(())
		}

		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn call_helper_function_from_template(origin: OriginFor<T>, value: u32) -> DispatchResult {
			// Check that the extrinsic was signed and get the signer.
			// This function will return an error if the extrinsic is not signed.
			// https://docs.substrate.io/v3/runtime/origins
			let _ = ensure_signed(origin)?;
			...
			// Emit an event.
			Self::deposit_event(Event::StoreUpdated(value));
			// Return a successful DispatchResultWithPostInfo
			Ok(())
		}
	}
